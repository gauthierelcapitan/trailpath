...
import { ElevationGeonamesResponseInterface } from '@trailpath/api/app/elevation/common/elevation-geonames-response.interface';
import { GEONAMES_ELEVATION_BATCH_SIZE } from '@trailpath/api/app/elevation/replace-elevation-geonames/replace-elevation-geonames.constant';
import { EnvironmentGeonamesInterface } from '@trailpath/api/environments/interface/environment-geonames.interface';
...
export class ReplaceElevationGeonamesService {
  ...
  private async fetchGeonamesElevation(coordinates: Position[], username: string): Promise<Position[]> {
    ...
    const { data } = await firstValueFrom(
      this.httpService
        .get<ElevationGeonamesResponseInterface>(url, {
          timeout: 10000,
        })
        .pipe(
